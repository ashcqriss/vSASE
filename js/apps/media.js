...
IOS.register({
  ...
  render(root) {
    ...
    function clipPlayer(clip) {
      ...
      v.muted = clip.muted !== false; // camera clips stay muted; drop-in movies get sound
      const bar = h("i");
      const playBtn = h("span", { class: "tb-ico", html: Glyphs.pause() });
    ...
    /* drop-in movies: videos/v1..v8.(mp4|webm) on disk are auto-detected and
       played in the real <video> element. The repo ships none — your local
       files live in the gitignored videos/ folder and never get pushed. */
    function probeLocalVideos(after) {
      const found = [];
      let pending = 0, done = false;
      const finish = () => { if (!done && pending === 0) { done = true; after(found); } };
      for (let i = 1; i <= 8; i++) {
        for (const ext of ["mp4", "webm"]) {
          pending++;
          const src = "videos/v" + i + "." + ext;
          const probe = document.createElement("video");
          probe.preload = "metadata";
          probe.muted = true;
          const settle = ok => {
            if (ok) found.push({ name: "v" + i, url: src,
              dur: isFinite(probe.duration) ? fmtDur(probe.duration) : "" });
            probe.src = ""; pending--; finish();
          };
      ...
      // nothing to probe / all synchronous-miss guard
    ...
    function paintList() {
      ...
      const localSec = h("div");
      list.append(localSec);
      probeLocalVideos(vids => {
        if (!vids.length) return;
        localSec.append(h("div", "ct-sec", "My Videos"));
        vids.sort((a, b) => a.name.localeCompare(b.name)).forEach(v =>
          localSec.append(h("div", { class: "store-row", onclick: () => { Snd.click(); clipPlayer({ name: v.name, url: v.url, muted: false }); } },
            h("div", { class: "store-app-ico", style: { background: "#2b2f38" }, html: Glyphs.play() }),
            h("div", "store-info", h("b", null, v.name),
              h("span", null, (v.dur ? v.dur + " · " : "") + "from videos/ · plays for real")))));
      });
